...
const uint32_t intervalo = 10000;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void loop() {
  // Transmisión automática cada 10 segundos
  if ((millis() - ultimo_envio) >= intervalo) {
    ...
    snprintf(txpacket, BUFFER_SIZE, "hola | T: %.1fC | H: %.1f%%", t, h);

    display.clear();
    display.drawString(0, 0, "Modo: auto tx");
    display.drawString(0, 16, "Enviando...");
    display.display();
  ...
  switch (state) {
    case ESTADO_TRANSMITIR:
      txNumber++;
      snprintf(txpacket, BUFFER_SIZE, "respuesta %d", txNumber);
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
  ...
  display.drawString(0, 16, "mensaje recibido:");
